package com.uoc.trainsystem.core.controller;

import java.util.List;

import javax.validation.Valid;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.uoc.trainsystem.core.request.AppRequest;
import com.uoc.trainsystem.core.request.CreateBookingTicketRequest;
import com.uoc.trainsystem.core.response.AppResponse;
import com.uoc.trainsystem.core.response.CreateBookingTicketResponseDTO;
import com.uoc.trainsystem.core.service.BookingTicketService;
import com.uoc.trainsystem.core.service.ViewBookingDetailsService;
import com.uoc.trainsystem.repository.wrapper.ViewBookingDetailsMappingDTO;

import io.swagger.annotations.ApiParam;

@RestController
@RequestMapping("/booking-details")
public class BookingTicketController {

	@Autowired
	private BookingTicketService bookingTicketService;
	@Autowired
	private ViewBookingDetailsService viewBookingDetailsService;
	
	@PostMapping("/create")
	public AppResponse<CreateBookingTicketResponseDTO> create (@Valid @RequestBody AppRequest<CreateBookingTicketRequest> req){
		try {
			CreateBookingTicketResponseDTO response = this.bookingTicketService.createBookingDetails(req.getData());
			return AppResponse.ok(response);
			
		} catch (Exception e) {	
			return AppResponse.error(HttpStatus.INTERNAL_SERVER_ERROR.toString(), e.getStackTrace()[0].getClassName(), 
					e.getStackTrace()[0].getMethodName(), "create booking details fail..");
		
		}
	}
	
	@GetMapping("/get/{userId}")
	public AppResponse<List<ViewBookingDetailsMappingDTO>> getDetails(@ApiParam(required = true) @PathVariable Long userId){
		try {
			List<ViewBookingDetailsMappingDTO> response = this.viewBookingDetailsService.getDetails(userId);
			return AppResponse.ok(response);
		} catch (Exception e) {	
			return AppResponse.error(HttpStatus.INTERNAL_SERVER_ERROR.toString(), e.getStackTrace()[0].getClassName(), 
					e.getStackTrace()[0].getMethodName(), "get booking details fail..");
		
		}
	}
	
}
